...
static GdkMonitor *window_get_target_monitor(int monitor_index) {
    ...
    if (monitor_index >= 0) {
        /* -1 sentinel intentionally keeps default monitor index. */
    ...
    /* Caller owns reference and is responsible for g_object_unref(). */
...
static int window_percent_to_margin(int percent, int monitor_size, unsigned int widget_size) {
    ...
    /* +50 implements integer rounding for percent conversion. */
...
/* Applies anchor-driven placement used when x/y percent are not configured. */
static void window_apply_anchor_placement(const WindowState *state) {
    GtkWindow *window = GTK_WINDOW(state->window);

    /* Switch keeps per-anchor edge/margin mapping explicit and predictable. */
