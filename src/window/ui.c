...
void window_build_widgets(WindowState *state) {
    ...
    /* Layout remains stable across themes: icon, progress bar, percentage text. */
    container = gtk_box_new(orientation, 0);
    /* Card class owns border/background styling in CSS theme. */
    ...
    if (is_vertical) {
        /* Vertical modifier class switches trough/progress sizing CSS. */
...
bool window_init_css(WindowState *state) {
    ...
    if (!state->args.css_replace) {
        /* Built-in provider establishes baseline card/slider styling. */
