...
static void set_status(OSDVolumeProcStatus *status, OSDVolumeProcError error, int exit_code, int term_signal, int spawn_error) {
    if (status == NULL) {
        return;
    }
...
static bool wait_for_child_process_with_timeout(pid_t child_pid, int *status, unsigned int timeout_ms, bool *timed_out) {
...
bool osd_volume_run_wpctl_get_volume_line(
...
read_complete:
    ...
    if (line_truncated) {
        if (!finalize_child_exit_status(child_pid, out_status)) {
            if (out_status->error == OSD_VOLUME_PROC_ERR_WAIT || out_status->error == OSD_VOLUME_PROC_ERR_READ_TIMEOUT) {
                return false;
            }
        }
