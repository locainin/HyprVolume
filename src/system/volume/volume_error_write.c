...
void osd_volume_write_resolve_error(FILE *err_stream, const OSDVolumePathStatus *status) {
  ...
  case OSD_VOLUME_PATH_ERR_NOT_FOUND:
    ...
    osd_volume_error_line_append_text(&line,
                                      "volume resolve failed: no trusted wpctl path found, set "
                                      "HYPRVOLUME_ALLOW_WPCTL_PATH_OVERRIDE=1 and HYPRVOLUME_WPCTL_PATH\n");
    break;
...
void osd_volume_write_proc_error(FILE *err_stream, const char *wpctl_path, OSDVolumePathSource source,
...
void osd_volume_write_parse_error(FILE *err_stream, const char *wpctl_path, OSDVolumePathSource source,
                                  const char *line, const OSDVolumeParseStatus *status) {
